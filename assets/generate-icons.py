...
def main():
    ...
    make_icon(bg=True, rounded=True, target_w=600, w=112, radius=220).convert('RGB').save(
        os.path.join(out, 'icon.png'))

    # 2. icon-only：透明底 + 全出血符号（大一点，无安全区限制）
    make_icon(bg=False, target_w=760, w=118).convert('RGB').save(
        os.path.join(out, 'icon-only.png'))

    # 3. adaptive foreground：透明底 + 居中安全区符号
    make_icon(bg=False, target_w=600, w=112).convert('RGB').save(
